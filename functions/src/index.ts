 ...
 */
export const issueUserQr = functions.https.onCall(
  async (data, context): Promise<IssueUserQrResponse> => {
    try {
      // 認証チェック
      if (!context.auth) {
        return {
 ...
 */
export const redeemPoints = functions.https.onCall(
  async (data: RedeemPointsRequest, context): Promise<RedeemPointsResponse> => {
    try {
      // 認証チェック
      if (!context.auth) {
        return {
      ...
      // スタッフ権限を検証
      const hasPermission = await verifyStaffPermission(staffId, storeId);
      if (!hasPermission) {
        return {
          success: false,
      ...
      const result = await db.runTransaction(async (transaction) => {
        ...
        const existingStamp = await transaction.get(
          stampHistoryRef.where('jti', '==', jti).limit(1)
        );
        ...
        // バッジ判定
        const newBadges = determineBadges(newGoldStamps);
